...
byte Dynamixel::readRaw(void){
  return p_dxl_serial->read();
}
void Dynamixel::writeRaw(uint8_t value){

	dxlTxEnable();

	p_dxl_serial->write(value);
...
byte Dynamixel::available(void){
  return p_dxl_serial->available();
}
void Dynamixel::dxlTxEnable(void){
  drv_dxl_tx_enable(num, TRUE);
}
void Dynamixel::dxlTxDisable(void){
  drv_dxl_tx_enable(num, FALSE);
}
...
#else
    p_dxl_serial->write(mTxBuffer, bPacketLength);
#endif
...
byte Dynamixel::rxPacket(int bRxLength){
  ...
  for(bCount = 0; bCount < bRxLength; bCount++)
  {
    while(1)
    {
      if( p_dxl_serial->available() )
      {
        mRxBuffer[bCount] = p_dxl_serial->read();
        break;
      }
